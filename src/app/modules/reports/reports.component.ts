...
export class ReportsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async getRoutes() {
    ...
    if (!localStorage.getItem('routes-actualRoute')) {
      localStorage.setItem('routes-actualRoute', JSON.stringify(this.routes.length ? this.routes[0] : null))
      this.reportsStorage.actualRoute = this.routes.length ? this.routes[0] : null
    } else {
      this.reportsStorage.actualRoute = JSON.parse(`${localStorage.getItem('routes-actualRoute')}`)
    }
  ...
  async changeRoute(event: any) {
    ...
    localStorage.setItem('routes-actualRoute', JSON.stringify(route))
    this.reportsStorage.actualRoute = route!
  }

  async consultReportsRange() {
  ...
  async getDataGraphics() {
    this.initDate = moment(this.formDates.value.initDate)
    this.finalDate = moment(this.formDates.value.finalDate)
  ...
  async getSalesReport() {
    try {
      const saleReport: SaleReport = await this.reportService.getSalesReport({
        route_id: this.reportsStorage.actualRoute!.id,
        initDate: this.initDate.format('YYYY-MM-DD'),
        finalDate: this.finalDate.format('YYYY-MM-DD'),
      });
